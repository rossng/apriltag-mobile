...
// SPDX-FileCopyrightText: 2019-2020 The CONIX Research Center (Wiselab CMU)
//
...
// see documentation in .h
EMSCRIPTEN_KEEPALIVE
int atagjs_set_family(const char* family_name)
{
    if (g_td == NULL) {
    ...
    // Remove current family from detector
    if (g_tf != NULL) {
        apriltag_detector_remove_family(g_td, g_tf);
        
        // Destroy current family based on its name
        if (strcmp(g_tf->name, "tag36h11") == 0) {
            tag36h11_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tag25h9") == 0) {
            tag25h9_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tag16h5") == 0) {
            tag16h5_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tagCircle21h7") == 0) {
            tagCircle21h7_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tagStandard41h12") == 0) {
            tagStandard41h12_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tagStandard52h13") == 0) {
            tagStandard52h13_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tagCircle49h12") == 0) {
            tagCircle49h12_destroy(g_tf);
        } else if (strcmp(g_tf->name, "tagCustom48h12") == 0) {
            tagCustom48h12_destroy(g_tf);
        }
        g_tf = NULL;
    }

    // Create new family based on family_name
    if (strcmp(family_name, "tag36h11") == 0) {
        ...
        g_tf = tag16h5_create();
    } else if (strcmp(family_name, "tagCircle21h7") == 0) {
        g_tf = tagCircle21h7_create();
    } else if (strcmp(family_name, "tagStandard41h12") == 0) {
        g_tf = tagStandard41h12_create();
    } else if (strcmp(family_name, "tagStandard52h13") == 0) {
        g_tf = tagStandard52h13_create();
    } else if (strcmp(family_name, "tagCircle49h12") == 0) {
        g_tf = tagCircle49h12_create();
    } else if (strcmp(family_name, "tagCustom48h12") == 0) {
        g_tf = tagCustom48h12_create();
    } else {
        printf("Error: Unknown tag family '%s'.\n", family_name);
        return -1;
    }

    if (g_tf == NULL) {
        printf("Error creating tag family '%s'.\n", family_name);
        return -1;
    }
...
t_str_json *atagjs_detect()
{
    ...
    {
        if (str_json_create(&g_det_json, 200) == 0) { // try to allocate string to return error string
